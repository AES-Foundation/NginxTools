...
Команда `upgrade` обновляет существующую установку NGINX, сохраняя все
пользовательские файлы: конфиги, сертификаты, .htpasswd, статику, логи
и наши сгенерированные `real_ip.conf` / `cloudflare_set_real_ip_from.conf`.
...
Команда поддерживает и понижение - выбирайте любую версию ниже текущей.
Полезно, если новая версия не подошла или возникла несовместимость с модулями.
...
* На Linux `upgrade` пока не поддерживается - там NGINX ставится через
  пакетный менеджер (`apt`, `dnf`) или собирается из исходников, и логика
  обновления другая. Поддержка появится в будущих версиях.

## Резервные копии и откат

Перед каждой командой `nginxtools upgrade` утилита автоматически создаёт
**полный снимок каталога NGINX** - включая `conf/`, `html/`, `logs/`,
`temp/`, сертификаты и всё остальное. Бэкап кладётся рядом с NGINX:
...
Если обновление упало - утилита автоматически восстанавливает NGINX из
только что созданного бэкапа. Ничего не теряется.
...
[!] **`removeOldBackups: true` означает: останется только один бэкап - самый свежий.**
Используйте, если у вас много инсталляций или ограничено место на диске.
...
При необходимости можно что-то достать руками, скопировать в рабочую
установку или удалить бэкап самостоятельно.
